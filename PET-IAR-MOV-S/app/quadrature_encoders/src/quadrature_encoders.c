...
#define ENCODER_KY040


// internal functions ! 
static volatile void gpio_callback_channel_ab(uint pin, uint32_t mask) ; 
...
//    gpio_set_irq_enabled(_port_channel_a,GPIO_IRQ_EDGE_FALL ,true); //| GPIO_IRQ_EDGE_RISE  ,true) ; 
//    gpio_set_irq_enabled(_port_channel_b,GPIO_IRQ_EDGE_FALL ,true); //| GPIO_IRQ_EDGE_RISE  ,true) ; 
        ...
        case STATE_00:           
            if (new_state == STATE_10){
                ...
                if (encoder.direction == COUNTER_STILL){
                    ...
                    encoder.direction = COUNTER_ANTICLOCKWISE ;
                    count_state = 0;        // ESTE RESETEO NO ESTABA - Lo agregue yo (capu) 
                }
...
static volatile void gpio_callback_channel_ab(uint gpio,uint32_t event_mask ) { 
    irq_count++ ;
    uint8_t new_state ; 
 
    if (gpio == _port_channel_a){
       new_state =encoder.state ; 
       new_state = new_state &0x01 ; 
       if (event_mask == GPIO_IRQ_EDGE_FALL){
           ...
           encoder.pulse_false_a++ ; 
       }

    }else if (gpio == _port_channel_b){
        new_state = encoder.state &0x02  ;
        new_state = new_state &0x02 ;  
        if (event_mask == GPIO_IRQ_EDGE_FALL){
           encoder.pulses_b++ ; 
